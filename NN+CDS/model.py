...
def train():
    df = pd.read_csv('descritors_by_rdkit.csv',header = 0)
    #print(descriptors)
    #labels = pd.read_csv('MP_REMOVED_ERROR.csv',header = 0).iloc[:,-2]
    #print(labels)
    #df = pd.concat([descriptors,labels],axis=1)
    test = df.sample(int(len(df)*0.1), random_state = 666)
    train = df.drop(test.index)
    validation = train.sample(int(len(df)*0.1), random_state = 666)
    ...
    X_val, Y_val = validation.iloc[:,:-2], validation.iloc[:,-2]
    ...
    NN = MLPRegressor(hidden_layer_sizes = (256,128), random_state=1, 
                      ...
                      n_iter_no_change=50, warm_start=True, learning_rate_init = 0.0003)
    mae_train = []
    mae_val = []
    mae_test = []
    r2_train = []
...
def evaluate():
    MODEL = load('MP.joblib')
    Scaler = load('scaler.joblib')
    indep_data = pd.read_csv('test_descriptor.csv',header = 0)
    X_indep, Y_indep = indep_data.iloc[:,0:-2], indep_data.iloc[:,-2]
